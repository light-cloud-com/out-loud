...
function synthesizeCopy(): Promise<void> {
...
async function onTrigger(): Promise<void> {
  const now = Date.now();
  if (triggering || now - lastTriggerAt < TRIGGER_COOLDOWN_MS) return;
  triggering = true;
  lastTriggerAt = now;
  try {
...
function doRegister(accelerator: string): void {
...
export function registerSelectionReader(
  ...
  accelerator: string = DEFAULT_READ_ALOUD_SHORTCUT
): void {
...
export function setSelectionShortcut(accelerator: string): void {
  const next = accelerator || DEFAULT_READ_ALOUD_SHORTCUT;
  if (next === currentShortcut && globalShortcut.isRegistered(currentShortcut)) return;
...
// Settings saves it).
export function suspendSelectionReader(): void {
  globalShortcut.unregister(currentShortcut);
}

export function resumeSelectionReader(): void {
  if (!globalShortcut.isRegistered(currentShortcut)) doRegister(currentShortcut);
}

export function unregisterSelectionReader(): void {
  globalShortcut.unregister(currentShortcut);
}
