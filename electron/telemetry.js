    ...
    for (const [key, value] of Object.entries(props)) {
        ...
        if (value === null) {
            ...
            if (value.length <= MAX_STR)
                out[key] = value; // drop over-long = possible content
        }
        else if (Array.isArray(value)) {
            const arr = value.filter((v) => typeof v === "number" || (typeof v === "string" && v.length <= MAX_STR));
            if (arr.length > 0)
                out[key] = arr.slice(0, MAX_ARR);
        }
...
export function lengthBucket(chars) {
    if (chars <= 0)
        return "0";
    if (chars <= 100)
        return "1-100";
...
// ---- init / persistence -----------------------------------------------------
export function initTelemetry(isDev) {
...
function enqueue(event, props) {
    ...
    if (queue.length > MAX_QUEUE)
        queue = queue.slice(-MAX_QUEUE); // drop oldest
...
export function trackFromRenderer(event, props) {
    if (mode === "off")
        return;
    if (typeof event !== "string" || event.length === 0 || event.length > 80)
        return;
    const safeProps = props && typeof props === "object" && !Array.isArray(props)
        ? props
        : {};
...
function sessionEndedProps(extra) {
...
export async function shutdownTelemetry(extra) {
    if (mode === "off")
        return;
    if (flushTimer) {
