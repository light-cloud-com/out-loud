 ...
 */
export function initCrashLog(file: string, header: Record<string, unknown> = {}): string | null {
 ...
 */
export function attachCrashLog(file: string): void {
  if (logPath) return;
  logPath = file;
}
...
export function logLine(scope: string, ...args: unknown[]): void {
  let msg: string;
  try {
    msg = format(...args);
  } catch {
 ...
 */
export function teeConsole(scope: string): void {
 ...
 */
export function installProcessHandlers(scope: string): void {
  process.on("uncaughtException", (err) => {
    logLine(`${scope}:fatal`, "uncaughtException:", err?.stack || err);
  });
  process.on("unhandledRejection", (reason) => {
    logLine(`${scope}:fatal`, "unhandledRejection:", (reason as Error)?.stack ?? reason);
  });
